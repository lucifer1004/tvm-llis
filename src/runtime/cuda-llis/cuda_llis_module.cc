...
class CUDALlisModuleNode : public runtime::ModuleNode {
 public:
  explicit CUDALlisModuleNode(std::string data, std::string fmt,
  ...
  const char* type_key() const final { return "cuda_llis"; }
...
class CUDALlisPrepGlobalBarrier {
 public:
  CUDALlisPrepGlobalBarrier(CUDALlisModuleNode* m, ObjectPtr<Object> sptr) : m_(m), sptr_(sptr) {
...
Module CUDALlisModuleLoadFile(const std::string& file_name, const std::string& format) {
  std::string data;
  std::unordered_map<std::string, FunctionInfo> fmap;
  std::string fmt = GetFileFormat(file_name, format);
  std::string meta_file = GetMetaFilePath(file_name);
  ...
  return CUDALlisModuleCreate(data, fmt, fmap, std::string());
}
...
TVM_REGISTER_GLOBAL("runtime.module.loadfile_ptx_llis").set_body_typed(CUDALlisModuleLoadFile);
