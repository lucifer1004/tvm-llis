...
void func(int num_streams, std::vector<int> sizes) {
    ...
    for (int i = 0; i < num_streams; ++i) {
        input_devs[i] = tvm::runtime::NDArray::Empty({1, 1, sizes[i], sizes[i]}, DLDataType{kDLFloat, 32, 1}, ctx_gpu);
        output_devs[i] = tvm::runtime::NDArray::Empty({1, 10}, DLDataType{kDLFloat, 32, 1}, ctx_gpu);
        std::string path = "pytorch_conv_";
        path += std::to_string(sizes[i]);
        path += "-pack-";
