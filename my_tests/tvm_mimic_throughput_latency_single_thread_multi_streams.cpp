...
void func(int num_streams) {
    ...
    for (int i = 0; i < num_streams; ++i) {
        input_devs[i] = tvm::runtime::NDArray::Empty({1, 1, 18}, DLDataType{kDLFloat, 32, 1}, ctx_gpu);
        output_devs[i] = tvm::runtime::NDArray::Empty({1, 2}, DLDataType{kDLFloat, 32, 1}, ctx_gpu);
        std::string path = "mimic-pack-";
